...
def build_maze(
    config: Config,
) -> tuple[MazeGenerator, list[list[int]], list[tuple[int, int]], str]:
...
def main() -> None:
    ...
    try:
        ...
        if config.WIDTH > 20 or config.HEIGHT > 15:
            raise ValueError("Maze size must be less than WIDTH=20, HEIGHT=15")
        ...
        while True:
            ...
            try:
                ...
                if columns < min_w or lines < min_h:
                    print(
                        f"\033[91mTerminal too small "
                        f"({columns}x{lines})!\033[0m"
                    )
            ...
            if choice == "1":
                ...
                print(
                    "Invalid choice, try again you have to choose "
                    "[1, 2, 3, 4]"
                )
